...
int main(int argc, char** argv){
	...
	if (argc!=7){
        ...
        err=posix_memalign((void**) &(counter), (size_t) INT_ALIGN, (size_t) (T+1)*sizeof(int));
        if(err!=0){
        	fprintf(stderr, "Problems in allocating memory to store counters\n");
        	exit(EXIT_FAILURE);
        } 
        for (int delta_t = 0; delta_t < (T); delta_t++) counter[delta_t]=0;

        //allocate memory to store data of the WHOLE sample
        // T+1 complex value for each values
        err=posix_memalign((void**) &Re_sample, (size_t) DOUBLE_ALIGN, (size_t)(T+1)*sizeof(double));
    	if(err!=0){
    	...
    	err=posix_memalign((void**) &Im_sample, (size_t) DOUBLE_ALIGN, (size_t)(T+1)*sizeof(double));
    	if(err!=0){
    	...
    	for(int delta_t=0; delta_t<(T);delta_t++){
    	...
    	err=posix_memalign((void**) &(Re_sample[(T)]), (size_t) DOUBLE_ALIGN, (size_t) (T)*num_data*sizeof(double));
        if(err!=0){
        
                fprintf(stderr, "Problems in allocating memory to store data sample\n");
                exit(EXIT_FAILURE);
        	}
        err=posix_memalign((void**) &(Im_sample[(T)]), (size_t) DOUBLE_ALIGN, (size_t) (T)*num_data*sizeof(double));
        if(err!=0){
        ...
        for (int i = 0; i< num_data; i++){
        	...
        	for (int t1 = 0; t1<T; t1++){
        		for(int t2 = 0; t2<=t1; t2++){
        			...
        			Re_sample[delta_t][counter[delta_t]] = (single_line[2*t1]*single_line[2*t2]+single_line[2*t1+1]*single_line[2*t2+1])/((double)(1*1*1));
        			Im_sample[delta_t][counter[delta_t]] = (single_line[2*t2]*single_line[2*t1+1]-single_line[2*t1]*single_line[2*t2+1])/((double)(1*1*1));
        			counter[delta_t]++;
        		}
        	}
        }
        ...
        for (int delta_t=0; delta_t<(T); delta_t++){
            ...
            // compute naive error (B=1)
            /*B=1;
            jack_knife_mean(Re_sample[(T)], (T)*num_data, (T)*B, res);
            naive_error=res[1];
            fprintf(fp, "%d   %d   %.15lg %.15lg\n", B, (T), res[0], res[1]);
            // compute error for B>1
            for (B=2; B<B_max; B++){
        ...
        for (int delta_t=0; delta_t<(T); delta_t++){
            ...
            // compute naive error (B=1)
            /*B=1;
            jack_knife_mean(Im_sample[(T)], (T)*num_data, (T)*B, res);
            naive_error=res[1];
            fprintf(fp, "%d   %d   %.15lg %.15lg\n", B, (T), res[0], res[1]);
            // compute error for B>1
            for (B=2; B<B_max; B++){
            ...
            for(int delta_t=0; delta_t<(T+1); delta_t++){
